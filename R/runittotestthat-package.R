...
#' 1. \code{\link{convert_test}} converts an individual \code{RUnit} test to a
#' call to a \code{testthat} test.
#' 2. \code{\link{convert_test_file}} sources all the tests in a file, converts 
#' each one, then outputs them to a file (or file connection; 
#' \code{\link[base]{stdout}} by default).
#' \code{\link{convert_package_tests}} takes a string naming a package, or a 
#' \code{devtools::package} object and loops over the files in `inst/tests` (or
#' wherever you specify), converting the tests, and outputting to files or 
